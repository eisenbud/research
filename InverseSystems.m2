...
export {"inverseSystem",
	...
	--the following should be renamed to fromDual and toDual
	"fromDu",
	"toDu",
...
toDu (ZZ, Matrix) := o->(d,M) -> toDu(d, ideal M)
...
inverseSystem Matrix := o-> M -> (
    ideal fromDu(M, DividedPowers => o.DividedPowers)
    )
...
doc ///
   ...
   Description
    Text
     If I is an ideal, then toDu(d,I) = inverseSystem(I, PowerBound =>d). 
...
doc ///
   ...
   Description
    ...
    Example
     ...
     f1 = x^2
     f2 = (x+y)^2
     betti res ideal fromDu f1
     betti res ideal fromDu f2
     betti res (I = ideal fromDu(f2, DividedPowers => false))
     I == inverseSystem f2
   Caveat
    The Base ring of the input must be a standard graded polynomial ring over a field.

    Logically, the output of fromDu should be an ideal, not a matrix, but the original
