...
check "K3Carpets"
viewHelp K3Carpets
---Experiments in trying to deform the carpet
betti res carpet(2,3)

S = ZZ/101[a,b,c,d,e,f,w,x,y,z]
mscroll = matrix"w,x,a,b,c;x,y,b,c,d"
betti res carpet(2,3,mscroll)
m1 = matrix"w,x,a,b,c;y,z,b,c,d" -- make the 2x2 block generic
betti res carpet(2,3,m1) - still Gorenstein
m2 = matrix"w,x,a,b,c;x,y,b,d,e" -- add a variable to the 2x3 block
betti res carpet(2,3,m2) -- not CM
pd = primaryDecomposition carpet(2,3,m2)
betti res pd_2
mgen = matrix"w,x,a,b,c;y,z,d,e,f"
betti res carpet(2,3,mgen)
pd = primaryDecomposition carpet(2,3,mgen)
betti res pd_0
betti res pd_1
--could it be that the 3,3 carpet is even infinitesimally rigid?
I = carpet(3,3)
H = Hom(I, (ring I)^1/I)
pH = prune H

--
(F,h0) = canonicalHomotopies(8,3,FineGrading=>true);
betti(F,Weights=>{1,1,0,0})
showQuadraticDegrees= (F, i)-> tally select(degrees F_i, d->d_0+d_1==i+1)
showQuadraticDegrees(F,3)
L1 = apply(numcols F.dd_1, i-> degree (F.dd_1)_i)
L0 = degrees ring F;length L0
L0
Lsum = flatten apply(length L0, a-> apply(length L1, b-> {a,b,L0_a+L1_b}))
viewHelp pairs
duples = L -> flatten apply(L,a->apply(L,b->{a,b}))
D=duples Lsum;
D_0
D' = select(duples Lsum, a->a_0_0 != a_1_0 and a_0_1 !=a_1_1 and a_0_2==a_1_2);
D'_0
(F.dd_1)_0 ** (F.dd_3)_0
(F.dd_1)_4 ** (F.dd_3)_6
length D'
