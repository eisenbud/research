...
N = coker schur({2,2},syz vars S);
N = coker schur({1,0},syz vars S);
N=schurModule({2,0}, ker vars S)
N.cache

res N
F = sheaf N
C=cohomologyTable(F, -5, 7)
C1=cohomologyTable(F(-2), -5, 7)
D= C+C1
...
topSupernaturalPart(CohomologyTally):= C -> (
	  ...
	  zeros:=sort toList difference(set(min tp..max tp), set tp);
	  pC:=pureCohomologyTable(zeros,  lowColumnNumber,highColumnNumber);
	  multiplier:=min apply(keys pC, k-> C_k/pC_k);
	  (multiplier, pC)
     )
topSupernaturalPart D



B=betti tateResolution (presentation N, E, -5, 5)
LD=set (lowestDegrees B)

viewHelp Set
sort toList( (difference (set(-5..8), LD))/minus)
...
--The following is from an old version...
