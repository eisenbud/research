...
export{canonicalMultipliers,getRootOfUnity,linearSeriesFromMultipliers,changeMultiplier,
     distinctPointsOfP1,randomPrymCanonicalNodalCuspidalCurve,randomPrymCanonicalNodalCurve,
     randomCanonicalNodalCuspidalCurve,artinianReduction,koszulMap,koszulMaps,
     criticalKoszulDegrees,verifyFarkasPrymGreenConjecture,Printing,randomDegreeDegKNodalCurve,
     verifyFarkasOnRandomNodalCurve}
...
randomDegreeDegKNodalCurve=method()

randomDegreeDegKNodalCurve(ZZ) := (g)->(
     (p,r):=getRootOfUnity(3,random(200));     
     kk:=ZZ/p;
     S:=kk[x_0,x_1];
...
verifyFarkasOnRandomNodalCurve=method(TypicalValue=>boolean,Options=>{Printing=>false})

verifyFarkasOnRandomNodalCurve(ZZ) := opt->(g) -> (
     (L,I):=randomDegreeDegKNodalCurve(g);
     (p,kk,S,PQ,A,s,T):=L;
     if opt.Printing then print(kk,p);
     (T1,I1):= artinianReduction(I);     
...
verifyFarkasPrymGreenConjecture(ZZ,ZZ,ZZ) := opt->(g,n,k) -> (
     (L,I):=randomPrymCanonicalNodalCurve(g,n,k);
     (r,p,kk,S,PQR,AB,s,T):=L;
     if opt.Printing then print(n,kk,r);
     (T1,I1):= artinianReduction(I);     
     assert(degree I1==2*g-2);
     d=(criticalKoszulDegrees I1)_0;
     assert(d==floor(g/2)-1);
     N:=koszulMap(I1,d-1);
     rk:=(g*binomial(g-3,d-2),(g-3)*binomial(g-3,d-1));
     if opt.Printing  then print (rk,betti N);
     M:=transpose N;  
     sM:=syz M;
     if rank source sM=!=0  then print (k,betti sM); 
     rank source sM==0)
...
doc ///
  ...
  Description
    Text
       ...
       appropiate step for a nodal or nodal-cuspidal rational curve of genus g which has either
       only nodes with k multipliers altered by root of unity or 3 cusps and g-3 nodes with all multipliers
       at the nodes altered.
       
       Step 1. Choice of a random Prym canonically embedded nodal-cuspidal or nodal curve.
    ...
    Example
       verifyFarkasPrymGreenConjecture(8,3,6,Printing=>true)
       g=8
...
doc ///
  ...
  Description
    Text
      ...
      @TO installPackage@("NodalCuspidalCurves4")

      {\bf Examples:}
///
doc ///
  Key
    randomDegreeDegKNodalCurve
    (randomDegreeDegKNodalCurve,ZZ)  
  Headline 
    Get a random nodal curve of genus g and degree 2g-2 
  Usage
    (L,I)=randomDegreeDegKlNodalCurve(g)
  Inputs
    g: ZZ
       the desired arithmetic genus
  Outputs
     L: Sequence 
     	a sequence of additionally created objects (kk,S,PQ,A,s,T) with kk=the ground field, 
	...
	s = the linear system, and T the coordinate ring of PP^{g-1}
     I: Ideal
        of the nodal degree 2g-2 curve
  
  Description
     Text
       Construct a rational curve with g double points of degree 2g-2.
       
       Step 1. Choose 2 times g points P_i,Q_i randomly in PP^1(kk) 
       which we indentify.
       
       Step 2. Choose random multipliers i.e. the ratio between the values of the sections 
       at P_i and Q_i.
       
       Step 3. Computer the homogeneous ideal I of the image curve under the linear system
     ...
     Example
         g=6;kappa=3;
         time (L,I)=randomDegreeDegKNodalCurve(g);
         (p,kk,S,PQ,A,s,T)=L;
         kk,S
	 PQ
	 A
...
doc ///
  ...
  Headline 
    Get a random Prym canonical nodal cuspidal curve of genus g, level n and k twisted multipliers 
  Usage
    (L,I)=randomPrymCanonicalNodalCuspidalCurve(g,n,k)
  Inputs
    g: ZZ
       the desired arithmetic genus
    n: ZZ
...
doc ///
  ...
  Outputs
	...
	s = the canonical system, and T the coordinate ring of PP^{g-2}
    I: Ideal
       of the random Prym canonical rational curve 
...
doc ///
  ...
  Inputs
       ...
       Q: Matrix
          coordinate matrix of delta points in P^1
       R: Matrix
          coordinate matrix of kappa points in P^1 if present
  Outputs
        : Matrix
        : Sequence
	  a matrix A or a pair (A,B) of matrices of multipliers
	  
          
  Description
     Text
       Given delta pairs of points P_i, Q_i, on P^1 and possibly kappa points R_j 
...
restart
uninstallPackage("NodalCuspidalCurves4")
installPackage("NodalCuspidalCurves4",RerunExamples=>true,RemakeAllDocumentation=>true);
viewHelp"NodalCuspidalCurves4"
setRandomSeed("alpha")
  
g=10
time apply(toList(3..g),k->(k,verifyFarkasPrymGreenConjecture(g,3,k,Printing=>true),betti res( (randomPrymCanonicalNodalCurve(g,3,k))_1) ))

time verifyFarkasOnRandomNodalCurve(12,Printing=>true) -- used 23.83 seconds
--time verifyFarkasOnRandomNodalCurve(14,Printing=>true) 
...
--time verifyFarkasPrymGreenConjecture(16,2,15,Printing=>true)
time verifyFarkasPrymGreenConjecture(16,4,15,Printing=>true)

time verifyFarkasOnRandomNodalCurve(16,Printing=>true) 
246613.0/60/60, 255220./60/60
end
