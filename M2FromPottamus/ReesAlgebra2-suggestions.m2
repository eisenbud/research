...
--Documenation
/// minimalReduction takes an ideal i that is homogeneous or inhomogeneous
...
such i is integrally dependent on j. 
...
j*i^k=i^(k+1) for some non-negative integer k.The smallest k with this
...
j*i^k=i^(k+1). The routine is probabilistic, since it depends on 
the routine "minimalReduction".
See the book 
Huneke, Craig; Swanson, Irena: Integral closure of ideals, rings, and modules. 
London Mathematical Society Lecture Note Series, 336. Cambridge University Press, Cambridge, 2006.
for further information.

See Also: which Gm, analyticSpread, minimalReduction
///

/// --Example for minimalReduction
restart
...
jhom=ideal (f*random(source f, S^{3-a:-2,a:-3}));
--print(i^2 == i*jhom);
print(i^6 == (i^5)*jhom);
)
///
...
--The minimal reduction here is necessarily inhomogeneous.
--There is apparently no homogeneous ideal with just 3 generators
--on which i is integrally dependent.
f= gens i
for a from 0 to 3 do(
...
codimension 0<k<m, the ideal i_P in S_P can be generated by at most k elements. The call
whichGm i
returns the largest m such that i satisfies G_m, or infinity if i satisfies G_m for every m.
...
See Also: minimalReduction, reductionNumber, analyticSpread
///
