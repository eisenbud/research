...
--
-- Example 4.1 starts after the first "end . 


sylvesterMatrix1=(n,x,y)->(R:=ring x;
...
--        returns a presentation matrix of the versal deformation.
     ...
     ee=matrix{{e_0,e_1}};
     xx=matrix{{x_0,x_1}};
     M:=sylvesterMatrix1(L_0,x_0,x_1);
     ...
     apply(1..#L-1,j->(
	       ...
	       NN=NN|sylvesterMatrix1(L_j,x_0,x_1);
...
apply(strata,s->(dim s, codim s, degree s, betti res s))
...
phi=map(R,B,uu|vv)
time Jo=ker phi  -- used 10.4964 seconds
Jo==J411
...
--phi=map(R,B,uu|vv)
--time J1=ker phi-- ,DegreeLimit=>5)

-- the special square in the filtration of (1,1,7)
restart
load "computingRpi_star.m2"
load "vectorBundlesOnPP1.m2"
L={1,3,5}
