...
--betti fL
     ...
     r1:=rank fL_1;
     r2:=rank fL_2;
     Aindex1:=toList (set(1..r0)**set(1..r1));
     Aindex:=select(toList Aindex1,ij->dfL0_(ij#0-1)<=dfL1_(ij#1-1));
     Bindex1:=toList (set(1..r1)**set(1..r2));
     Bindex:=select(toList Bindex1,ij->dfL1_(ij#0-1)<=dfL2_(ij#1-1));
     A:=kk[apply(Aindex,ij->a_ij),Degrees=>apply(Aindex,ij->dfL1_(ij#1-1)-dfL0_(ij#0-1)+1)]; 
     -- ** deg+1: a work around no degree zero variables
     mA:=map(A^(-degrees fL_0),A^(-degrees fL_1)**A^{-1},
	  matrix apply(toList(1..r0),i->apply(toList(1..r1),j->if dfL0_(i-1)<=dfL1_(j-1) then a_(i,j) else 0_A)));
     
     B:=kk[apply(Bindex,ij->b_ij),Degrees=>apply(Bindex,ij->dfL2_(ij#1-1)-dfL1_(ij#0-1)+1)];
     mB:=map(B^(-degrees fL_1),B^(-degrees fL_2)**B^{-1},
	  matrix apply(toList(1..r1),i->apply(toList(1..r1),j->if dfL1_(i-1)<=dfL1_(j-1) then b_(i,j) else 0_B)));
     AB:=A**B;
     AR:=AB**R;
  --   isHomogeneous mA;
...
--betti transpose  eq1
betti(eqM1:=diff(transpose (flatten mA1|flatten mB1),eq1));
--eqM1
     -- correction for **:
betti(eqM:=map(target eqM1,(source eqM1)**AR^{-1},eqM1));
 --    isHomogeneous eqM
betti(     sol1:=syz transpose substitute(eqM,R));
--betti sol1
--betti eqM
...
--betti trivC
     ...
     --betti trivE
     --betti trivC
     --betti gb truncate(-1,image (trivC|trivE))
     --isHomogeneous trivE
--betti truncate(-1,image sol1A)
--betti  (gens truncate(-1,image sol1A)%gb image (trivC|trivE))
betti(     def:=mingens image (gens truncate(-1,image sol1A)%gb image (trivC|trivE)));
     tt:=rank source def;
     T=kk[t_1..t_tt];
     RT=R**T;
     defT:=substitute(def,RT)*transpose substitute(vars T,RT);
...
L2=(substitute(a0+a1-aa2,RT),substitute(b0+b1-bb2,RT));
     )
)
end

restart
load "deformations.m2"
kk=ZZ/101
S=kk[x,y,z]
