...
Ideal ^ Array := (I,d) -> ideal apply(I_*, f -> f^(first d))
kk=ZZ/101
n=2
B = kk[x_1..x_n,y_1..y_n]
time for p from 1 to 100 list(
     I = (ideal vars B)^[4]+ 
...
time for p from 1 to 1000 list(
     I = (ideal vars B)^[8]+ 
     randomMonomialIdeal({4,5,6},B)+
     randomBinomialIdeal({4,4,4},B)+
     randomIdeal(vars B,{3,3});
     print netList {I};
     b := isEquivFromMap I;
     print b;
     b
...
use B
I = (ideal(x_1,x_2,y_1,y_2))^3
Ie = makeEquiv I
A = kk[x_1..x_n]
use B
mapFromEquivalence(Ie,A)

A = kk[x_1]
L = {x_1^5, x_1^7}
L = {x_1^5, x_1^11, x_1^12}
L = {x_1^5, x_1^8, x_1^11, x_1^12}

  B = kk[x_1,y_1];

-- BUG:
for d from 4 to 20 list (
  if d % 3 == 0 then continue;
  use A;
  L = {x_1^3, x_1^d};
  J = equivalenceFromMap(L,B);
  mapFromEquivalence(30,J,A))
----------------------------

for d from 4 to 20 list (
  if d % 5 == 0 then continue;
  use A;
  L = {x_1^5, x_1^d};
  use B;
  J = equivalenceFromMap(L,B);
  (d,mapFromEquivalence(30,J,A)))
netList oo
